...
import com.fluentcommerce.rule.order.allocation.otcitems.FulfilRemainingProductsFromOtcNetwork;
...
/**
 @author Nandhakumar
 */
public class FulfilRemainingProductsFromOtcNetworkTest extends BaseTest {
    private FulfilRemainingProductsFromOtcNetwork rule;
    private static String filePath = PATH_PREFIX + FULFIL_REMAINING_PRODUCTS_FROM_OTC_NETWORK + BACK_SLASH
            + SCENARIO_1 + BACK_SLASH;

    @Before
    public void setUp() {
        rule = new FulfilRemainingProductsFromOtcNetwork();
    }

    @Test
    public void testClass() throws Exception {
        String className = FulfilRemainingProductsFromOtcNetwork.class.getSimpleName();
        mockEvent();
