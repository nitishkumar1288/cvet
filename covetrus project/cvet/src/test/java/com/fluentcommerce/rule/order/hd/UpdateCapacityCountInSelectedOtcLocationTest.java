...
import com.fluentcommerce.rule.order.allocation.otcitems.UpdateCapacityCountInSelectedOtcLocation;
...
/**
 @author Nandhakumar
 */
public class UpdateCapacityCountInSelectedOtcLocationTest extends BaseTest {
    private UpdateCapacityCountInSelectedOtcLocation rule;
    private static String filePath = PATH_PREFIX + UPDATE_CAPACITY_COUNT_IN_SELECTED_OTC_LOCATION + BACK_SLASH
            + SCENARIO_1 + BACK_SLASH;

    @Before
    public void setUp() {
        rule = new UpdateCapacityCountInSelectedOtcLocation();
    }

    @Test
    public void testClass() throws Exception {
        String className = UpdateCapacityCountInSelectedOtcLocation.class.getSimpleName();
        mockEvent();
