...
import com.fluentcommerce.rule.order.allocation.otcitems.SendEventToUpdateInventoryQuantityFromOtcOrderItems;
...
public class SendEventToUpdateInventoryQuantityFromOtcOrderItemsTest extends BaseTest {
    private SendEventToUpdateInventoryQuantityFromOtcOrderItems rule;
    private static String filePath = PATH_PREFIX + SEND_EVENT_TO_UPDATE_INVENTORY_QUANTITY_FROM_OTC_ORDER_ITEMS+BACK_SLASH + SCENARIO_1
            + BACK_SLASH;

    @Before
    public void setUp() {
        rule = new SendEventToUpdateInventoryQuantityFromOtcOrderItems();
    }

    @Test
    public void testClass() throws Exception {
        String className = SendEventToUpdateInventoryQuantityFromOtcOrderItems.class.getSimpleName();
        //mock data
        mockEvent();
    ...
    private void mockEvent() {
        ...
        try{
            ...
            List<OtcProductsDto> otcProductsDtoList = mapper.readValue(
                    eventAttributes.get(EVENT_OTC_ORDER_ITEMS_ALLOCATION).toString(),
                    new TypeReference<List<OtcProductsDto>>(){});
