...
import com.fluentcommerce.rule.order.common.ordernotes.CreateOrderNotes;
...
public class CreateOrderNotesTest extends BaseTest {
    private CreateOrderNotes rule;
    private static  String filePath = PATH_PREFIX + CREATE_ORDER_NOTES +
            BACK_SLASH + SCENARIO_1 + BACK_SLASH;

    @Before
    public void setUp() {
        rule = new CreateOrderNotes();
    }
    ...
    private void mockParameters() {
        ...
        eventAttributes.add(USER_KEY);
        eventAttributes.add(NOTE_CONTEXT);
        eventAttributes.add(NOTE);
