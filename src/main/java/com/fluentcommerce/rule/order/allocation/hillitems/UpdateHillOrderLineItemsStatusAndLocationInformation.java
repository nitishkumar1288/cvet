...
@RuleInfo(
        name = "UpdateHillOrderLineItemsStatusAndLocationInformation",
...
@Slf4j
public class UpdateHillOrderLineItemsStatusAndLocationInformation extends BaseRule {
    ...
    public void run(ContextWrapper context) {
        ...
        for( HillProductsDto hillProductsDto: hillProductsDtoList) {
            ...
            itemAttributes.add(
                    AttributeInput.builder().name(FACILITY_ID).type(ATTRIBUTE_TYPE_STRING).value(hillProductsDto.getLocationId()).build()
            );

            if (hillProductsDto.isDefaultLocationSelected()) {

                itemAttributes.add(
                        AttributeInput.builder().name(DEFAULT_SELECTED_LOCATION).type(ATTRIBUTE_TYPE_STRING).value(hillProductsDto.getFoundLocation()).build()
                );

            } else {

                itemAttributes.add(
                        AttributeInput.builder().name(SELECTED_LOCATION).type(ATTRIBUTE_TYPE_STRING).value(hillProductsDto.getFoundLocation()).build()
                );
            ...
            updateOrderItems.add(
                    UpdateOrderItemWithOrderInput
                            .builder()
                            .id(hillProductsDto.getOrderItemId())
