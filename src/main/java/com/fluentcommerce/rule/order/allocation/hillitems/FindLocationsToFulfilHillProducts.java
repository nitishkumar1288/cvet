...
@RuleInfo(
        ...
        description = "Find the locations to fulfil the hill items and sends event {" + PROP_EVENT_NAME + "}.",
        produces = {
                @EventInfo(
                        eventName = "{eventName}",
...
public class FindLocationsToFulfilHillProducts extends BaseRule {
    ...
    public void run(ContextWrapper context) {
        ...
        List<HillProductsDto> hillProductsWithKit = hillProductsDtoList.stream().filter(
                hillProductsDto -> (hillProductsDto.isKitProduct())
        ).collect(Collectors.toList());

        List<HillProductsDto> hillProductsWithoutKit = hillProductsDtoList.stream().filter(
                hillProductsDto -> !(hillProductsDto.isKitProduct())
        ).collect(Collectors.toList());

        List<HillProductsDto> combinedHillProducts = new ArrayList<>();
        combinedHillProducts.addAll(hillProductsWithoutKit);
        combinedHillProducts.addAll(hillProductsWithKit);

        hillProductsDtoList = combinedHillProducts;
        ...
        List<String> orderItemIdsToFulfil = hillProductsDtoList.stream().map(
                HillProductsDto::getOrderItemId
        ) .collect(Collectors.toList());
        ...
        while(iterateTillLocationsAreFound){
            ...
            if(maximumProductFulfilledInLocation > 0){
                context.addLog("maximumProductFulfilledInLocation " + maximumProductFulfilledInLocation);
    ...
    private static int handleRemainingItems(
        ...
        for(HillProductsDto hillProductsDto : hillProductsDtoList) {
            ...
            if(orderItemIdsToFulfil.contains(hillProductsDto.getOrderItemId())) {
    ...
    private static int getProductsFulfilledInLocation(
        ...
        if(hillProductsDto.isKitProduct()){
            context.addLog("order item id : " + hillProductsDto.getOrderItemId() + " is a kit product");

            List<KitItem> kitItemList = hillProductsDto.getKitItemList();
            ...
            if (locationSelected) {
                ...
                orderItemIdList.add(hillProductsDto.getOrderItemId());
            }
        }
        else{
            // handle the non kit products
            // check whether the product is part of the Vp map and check whether the requested quantity is present
            if (vpMapForLocation.containsKey(hillProductsDto.getProductRef()) &&
                    vpMapForLocation.get(hillProductsDto.getProductRef()) >= hillProductsDto.getRequestedQuantity()) {

                productsFulfilledInLocation = productsFulfilledInLocation + 1;

                //reduce the VP
                vpMapForLocation.put(hillProductsDto.getProductRef(), vpMapForLocation.get(hillProductsDto.getProductRef()) - hillProductsDto.getRequestedQuantity());
    ...
    private static void handleVpForKitAndNonKitItems(
        ...
        for(HillProductsDto hillProductsDto : hillProductsDtoList) {
            if(selectedOrderItemIdList.contains(hillProductsDto.getOrderItemId())){
                ...
                orderItemIdsToFulfil.remove(hillProductsDto.getOrderItemId());

                // add the location information to the dto
                hillProductsDto.setFoundLocation(selectLocationVpList.get(0).getGroupRef());
                ...
                if(optionalFacilityId.isPresent()){
                    hillProductsDto.setLocationId(optionalFacilityId.get());
                }
            }
        }
    }
    ...
    private static void handleVpInNonKitItem(
        ...
        Optional<VirtualPosition> optionalVirtualPosition =
                selectLocationVpList.stream().filter(
                        virtualPosition -> virtualPosition.getProductRef().equalsIgnoreCase(hillProductsDto.getProductRef())
                ).findFirst();
        if(optionalVirtualPosition.isPresent()){
            optionalVirtualPosition.get().setQuantity(
                    optionalVirtualPosition.get().getQuantity() - hillProductsDto.getRequestedQuantity()
            );
        }else{
            context.addLog("logical error since vp checks are already made for normal product : " + hillProductsDto.getProductRef());
        }
    }
