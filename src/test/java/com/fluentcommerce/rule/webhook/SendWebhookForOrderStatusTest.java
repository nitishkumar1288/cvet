...
import com.fluentcommerce.rule.webhook.order.SendWebhookForOrderStatus;
...
import static com.fluentcommerce.util.Constants.ORDER_STATUS_PAYLOAD;
import static com.fluentcommerce.util.Constants.PROP_EVENT_NAME;
import static org.mockito.Mockito.when;

public class SendWebhookForOrderStatusTest extends BaseTest {
    private SendWebhookForOrderStatus rule;
    private static  String filePath = PATH_PREFIX_WEBHOOK + SEND_WEBHOOK_FOR_ORDER_STATUS + BACK_SLASH +
            SCENARIO_1 + BACK_SLASH;

    @Before
    public void setUp() {
        rule = new SendWebhookForOrderStatus();
    }

    @Test
    public void testClass() throws Exception{
        String className =  SendWebhookForOrderStatus.class.getSimpleName();
        // SETUP
        mockEvent();
    ...
    private void mockEvent() throws IOException {
        ...
        try{
            OrderStatusPayload paymentData = mapper.readValue(attJsonObject.get(ORDER_STATUS_PAYLOAD)
                    .toString(), new TypeReference<OrderStatusPayload>(){});
            attributeMap.put(ORDER_STATUS_PAYLOAD, paymentData);
