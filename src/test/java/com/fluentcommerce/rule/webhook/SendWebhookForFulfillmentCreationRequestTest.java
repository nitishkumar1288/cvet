...
import com.fluentcommerce.rule.webhook.order.SendWebhookForFulfillmentCreationRequest;
...
import static com.fluentcommerce.util.Constants.FULFILLMENT_REQUEST_PAYLOAD;
import static com.fluentcommerce.util.Constants.PROP_EVENT_NAME;
import static org.mockito.Mockito.when;

public class SendWebhookForFulfillmentCreationRequestTest extends BaseTest {
    private SendWebhookForFulfillmentCreationRequest rule;
    private static  String filePath = PATH_PREFIX_WEBHOOK + SEND_WEBHOOK_FOR_FULFILLMENT_CREATION_REQUEST + BACK_SLASH +
            SCENARIO_1 + BACK_SLASH;

    @Before
    public void setUp() {
        rule = new SendWebhookForFulfillmentCreationRequest();
    }

    @Test
    public void testClass() throws Exception{
        String className =  SendWebhookForFulfillmentCreationRequest.class.getSimpleName();
        // SETUP
        mockEvent();
    ...
    private void mockEvent() throws IOException {
        ...
        try{
            FulfillmentRequestData requestData = mapper.readValue(attJsonObject.get(FULFILLMENT_REQUEST_PAYLOAD)
                    .toString(), new TypeReference<FulfillmentRequestData>(){});
            attributeMap.put(FULFILLMENT_REQUEST_PAYLOAD, requestData);
