...
import com.fluentcommerce.rule.webhook.fulfilment.SendPaymentSettlementRequestWebhook;
...
import static com.fluentcommerce.util.Constants.PAYMENT_SETTLEMENT_REQUEST;
import static org.mockito.Mockito.when;

public class SendPaymentSettlementRequestWebhookTest extends BaseTest {
    private SendPaymentSettlementRequestWebhook rule;
    private static  String filePath = PATH_PREFIX_WEBHOOK + SEND_PAYMENT_SETTLEMENT_REQUEST_WEBHOOK + BACK_SLASH +
            SCENARIO_1 + BACK_SLASH;

    @Before
    public void setUp() {
        rule = new SendPaymentSettlementRequestWebhook();
    }

    @Test
    public void testClass() throws Exception{
        String className =  SendPaymentSettlementRequestWebhook.class.getSimpleName();
        // SETUP
        mockEvent();
    ...
    private void mockEvent() throws IOException {
        ...
        try{
            PaymentSettlementRequest paymentData = mapper.readValue(attJsonObject.get(PAYMENT_SETTLEMENT_REQUEST)
                    .toString(), new TypeReference<PaymentSettlementRequest>(){});
            attributeMap.put(PAYMENT_SETTLEMENT_REQUEST, paymentData);
