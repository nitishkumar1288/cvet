...
public class UpdateOrderAttributeAfterAppeasementCreatedTest extends BaseTest {
    private UpdateOrderAttributeAfterAppeasementCreated rule;
    private static  String filePath = PATH_PREFIX_CREDIT_MEMO + UPDATE_ORDER_ATTRIBUTE_AFTER_APPEASEMENT_CREATED + BACK_SLASH +
            SCENARIO_1 + BACK_SLASH;

    @Before
    public void setUp() {
        rule = new UpdateOrderAttributeAfterAppeasementCreated();
    }

    @Test
    public void testClass() throws Exception{
        String className =  UpdateOrderAttributeAfterAppeasementCreated.class.getSimpleName();
        // SETUP
        mockEvent();
        ...
        rule.run(context);
        //VERIFICATION
        VerificationUtils.verifyMutationClass(UpdateCreditMemoMutation.class,context,1);
    }
    ...
    private void mockEvent() throws IOException {
        ...
        double data = Double.parseDouble((String )attJsonObject.get("amountToBeRefund"));
        attributes.put("amountToBeRefund",data);
