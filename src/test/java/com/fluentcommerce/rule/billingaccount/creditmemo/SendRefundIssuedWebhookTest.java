...
import com.fluentcommerce.rule.webhook.creditmemo.SendRefundIssuedWebhook;
...
public class SendRefundIssuedWebhookTest extends BaseTest {
    private SendRefundIssuedWebhook rule;
    private static  String filePath = PATH_PREFIX_CREDIT_MEMO + SEND_REFUND_ISSUED_WEBHOOK + BACK_SLASH +
            SCENARIO_1 + BACK_SLASH;

    @Before
    public void setUp() {
        rule = new SendRefundIssuedWebhook();
    }

    @Test
    public void testClass() throws Exception{
        String className =  SendRefundIssuedWebhook.class.getSimpleName();
        // SETUP
        mockEvent();
    ...
    private void mockEvent() throws IOException {
        ...
        try{
            RefundProcessed refundProcessed = mapper.readValue(attJsonObject.get(PROCESSED_REFUND_DATA)
                    .toString(), new TypeReference<RefundProcessed>(){});

            attributeMap.put(PROCESSED_REFUND_DATA, refundProcessed);
