...
import com.fluentcommerce.rule.order.allocation.rxitems.SoftReserveForPreAssignedFacilityOrderItems;
...
import static com.fluentcommerce.util.Constants.EVENT_ALL_VP_INFORMATION;
...
/**
 @author Nandhakumar
 */
public class SoftReserveForPreAssignedFacilityOrderItemsTest extends BaseTest {
    private SoftReserveForPreAssignedFacilityOrderItems rule;
    private static String filePath = PATH_PREFIX + SOFT_RESERVE_FOR_PRE_ASSIGNED_FACILITY_ORDER_ITEMS + BACK_SLASH
            + SCENARIO_1 + BACK_SLASH;

    @Before
    public void setUp() {
        rule = new SoftReserveForPreAssignedFacilityOrderItems();
    }

    @Test
    public void testClass() throws Exception {
        String className = SoftReserveForPreAssignedFacilityOrderItems.class.getSimpleName();
        mockEvent();
    ...
    private void mockEvent() {
        ...
        try{
            List<VirtualPosition> virtualPositionList  = mapper.readValue(eventAttributes.get(EVENT_ALL_VP_INFORMATION).toString(), new TypeReference<List<VirtualPosition>>(){});
            attributeMap.put(EVENT_ALL_VP_INFORMATION, virtualPositionList);
        } catch (Exception exception){
            context.addLog("Exception Occurred when mapping " + exception.getMessage());
            return;
        }
