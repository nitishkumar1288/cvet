...
import com.fluentcommerce.rule.order.allocation.otcitems.AllocateUnassignedOtcItemsToDefaultLocation;
...
/**
 @author Nandhakumar
 */
public class AllocateUnassignedOtcItemsToDefaultLocationTest extends BaseTest {
    private AllocateUnassignedOtcItemsToDefaultLocation rule;
    private static String filePath = PATH_PREFIX + ALLOCATE_UNASSIGNED_OTC_ITEMS_TO_DEFAULT_LOCATION + BACK_SLASH
            + SCENARIO_1 + BACK_SLASH;

    @Before
    public void setUp() {
        rule = new AllocateUnassignedOtcItemsToDefaultLocation();
    }

    @Test
    public void testClass() throws Exception {
        String className = AllocateUnassignedOtcItemsToDefaultLocation.class.getSimpleName();
        mockEvent();
    ...
    private void mockParameters(String className) {
        when(context.getProp(PROP_DEFAULT_HILL_LOCATION)).thenReturn(VFC_WAREHOUSE);
