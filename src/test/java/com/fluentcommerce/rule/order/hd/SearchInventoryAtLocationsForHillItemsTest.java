...
import com.fluentcommerce.rule.order.allocation.hillitems.SearchInventoryAtLocationsForHillItems;
...
/**
 @author Nandhakumar
 */
public class SearchInventoryAtLocationsForHillItemsTest extends BaseTest {
    private SearchInventoryAtLocationsForHillItems rule;
    private static String filePath = PATH_PREFIX + SEARCH_INVENTORY_AT_LOCATIONS_FOR_HILL_ITEMS + BACK_SLASH
            + SCENARIO_1 + BACK_SLASH;

    @Before
    public void setUp() {
        rule = new SearchInventoryAtLocationsForHillItems();
    }

    @Test
    public void testClass() throws Exception {
        String className = SearchInventoryAtLocationsForHillItems.class.getSimpleName();
        mockEvent();
    ...
    private void mockEvent() {
        ...
        String hillOrderItemAllocation = sceneBuilder.readTestResource(filePath + EVENT_ATTRIBUTES);
        JSONObject eventJson = new JSONObject(hillOrderItemAllocation);
        ObjectMapper mapper = new ObjectMapper();
        try{

            List<HillProductsDto> hillProductsDtoList = mapper.readValue(eventJson.get(EVENT_HILL_ORDER_ITEMS_ALLOCATION).toString(), new TypeReference<List<HillProductsDto>>(){});
            attributeMap.put(EVENT_HILL_ORDER_ITEMS_ALLOCATION, hillProductsDtoList);

            Map<String, LocationDto> locations = mapper.readValue(eventJson.get(EVENT_FIELD_LOCATIONS).toString(), new TypeReference<Map<String, LocationDto>>(){});
