...
import com.fluentcommerce.rule.webhook.order.SendPaymentAuthReversalWebhook;
...
public class SendPaymentAuthReversalWebhookTest extends BaseTest {
    private SendPaymentAuthReversalWebhook rule;
    private static  String filePath = PATH_PREFIX_WEBHOOK + SEND_PAYMENT_AUTH_REVERSAL_WEBHOOK + BACK_SLASH +
            SCENARIO_1 + BACK_SLASH;

    @Before
    public void setUp() {
        rule = new SendPaymentAuthReversalWebhook();
    }

    @Test
    public void testClass() throws Exception{
        String className =  SendPaymentAuthReversalWebhook.class.getSimpleName();
        // SETUP
        mockEvent();
    ...
    private void mockEvent() throws IOException {
        ...
        try{
            PaymentAuthReversalRequest paymentData = mapper.readValue(attJsonObject.get(PAYMENT_AUTH_REVERSAL_REQUEST_FOR_ORDER)
                    .toString(), new TypeReference<PaymentAuthReversalRequest>(){});
            attributeMap.put(PAYMENT_AUTH_REVERSAL_REQUEST_FOR_ORDER, paymentData);
