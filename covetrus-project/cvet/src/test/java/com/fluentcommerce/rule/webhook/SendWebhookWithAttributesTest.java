...
import com.fluentcommerce.rule.webhook.order.SendWebhookWithAttributes;
...
public class SendWebhookWithAttributesTest extends BaseTest {
    private SendWebhookWithAttributes rule;
    private static  String filePath = PATH_PREFIX_WEBHOOK + SEND_WEBHOOK_WITH_ATTRIBUTES + BACK_SLASH +
            SCENARIO_1 + BACK_SLASH;

    @Before
    public void setUp() {
        rule = new SendWebhookWithAttributes();
    }

    @Test
    public void testClass() throws Exception{
        String className =  SendWebhookWithAttributes.class.getSimpleName();
        // SETUP
        mockEvent();
    ...
    private void mockEvent() throws IOException {
        ...
        try{
            List<OrderNotes> orderNotesList = mapper.readValue(attJsonObject.get(ORDER_NOTES)
                    .toString(), new TypeReference<List<OrderNotes>>(){});
            attributeMap.put(ORDER_NOTES, orderNotesList);
