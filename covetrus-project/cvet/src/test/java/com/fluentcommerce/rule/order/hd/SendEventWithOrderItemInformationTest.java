...
import static com.fluentcommerce.util.Constants.PROP_ITEM_TYPE_LIST;
import static org.mockito.Mockito.when;

/**
     @author Nandhakumar
*/
public class SendEventWithOrderItemInformationTest extends BaseTest {
    private SendEventWithOrderItemInformation rule;
    private static String filePath = PATH_PREFIX + SEND_EVENT_WITH_ORDER_ITEM_INFORMATION + BACK_SLASH
            + SCENARIO_1 + BACK_SLASH;

    @Before
    public void setUp() {
        rule = new SendEventWithOrderItemInformation();
    }
    ...
    private void mockParameters(String className) {
        ...
        itemTypeList.add(IS_STERILE);
        itemTypeList.add(IS_COMPOUND);
        itemTypeList.add(IS_CONTROLLED);
        when(context.getPropList(PROP_ITEM_TYPE_LIST,String.class)).thenReturn(itemTypeList);
    }
