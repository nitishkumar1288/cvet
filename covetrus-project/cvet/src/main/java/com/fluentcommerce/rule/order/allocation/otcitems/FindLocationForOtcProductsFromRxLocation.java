...
@Slf4j
public class FindLocationForOtcProductsFromRxLocation extends BaseRule {
    ...
    public void run(ContextWrapper context) {
        ...
        // step 1 : Get the location list from the Rx allocation dto and there is no priority within Rx locations
        List<String> locationRefListOfRxItems =
        ...
        while (iterateTillLocationsAreFound) {
            ...
            for (String location : locationRefListOfRxItems) {

                List<VirtualPosition> locationSpecificVpList =
                        virtualPositionList.stream().filter(
                                virtualPosition -> virtualPosition.getGroupRef().equalsIgnoreCase(location)
                        ).collect(Collectors.toList());
                ...
                if (CollectionUtils.isNotEmpty(locationSpecificVpList)) {
                    ...
                    context.addLog("locationRefWithCapacity.get(location.getRef()) " + locationRefWithCapacity.get(location));
            ...
            iterateTillLocationsAreFound = checkAllItemsAreFulfilled(
                    context,
                    iterateTillLocationsAreFound,
                    orderItemIdsToFulfil);
    ...
    private static int getProductsCountPostCapacityCheck(
            ...
            String location,
            int productsFulfilledInLocation) {
        if ((locationRefWithCapacity.get(location)) < productsFulfilledInLocation) {
            productsFulfilledInLocation = locationRefWithCapacity.get(location);
            context.addLog("modified productsFulfilledInLocation " + productsFulfilledInLocation);
        }
        return productsFulfilledInLocation;
    }

    private static void handleVpForKitAndNonKitItems(ContextWrapper context, Map<String, Integer> locationRefWithCapacity, List<PharmacyAllocationForOrderItems> pharmacyAllocationForOrderItemsList, List<OtcProductsDto> otcProductsDtoList, List<String> orderItemIdsToFulfil, List<VirtualPosition> selectLocationVpList, List<String> selectedOrderItemIdList) {
        for (OtcProductsDto otcProductsDto : otcProductsDtoList) {
            if (selectedOrderItemIdList.contains(otcProductsDto.getOrderItemId())) {
                ...
                Optional<String> optionalFacilityId =
                        pharmacyAllocationForOrderItemsList.stream().filter(
                                dto -> dto.getAssignedFacilityRef().equalsIgnoreCase(finalSelectLocationVpList.get(0).getGroupRef())
                        ).map(PharmacyAllocationForOrderItems::getAssignedFacilityId).findFirst();
    ...
    private static boolean checkAllItemsAreFulfilled(
            ContextWrapper context,
            boolean iterateTillLocationsAreFound,
            List<String> orderItemIdsToFulfil) {
        ...
        return iterateTillLocationsAreFound;
    }
}
