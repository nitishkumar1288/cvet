...
@RuleInfo(
        name = "SendEventBasedOnRefundType",
        description = "check for the refund type  and send the event {" + PROP_EVENT_NAME + "} if match the condition " +
...
@Slf4j
public class SendEventBasedOnRefundType extends BaseRule {
    @Override
    public void run(ContextWrapper context) {
        ...
        if (null != refundResponse && null != refundResponse.getRefundKey()) {
            ...
            if (null != creditMemo){
                if (null != creditMemo.returnOrder() && null != creditMemo.returnOrder().ref()){
                    EventUtils.forwardInline(context,eventName);
                }else {
                    EventUtils.forwardInline(context,noMatchEventName);
                }
            }else{
                context.addLog("SendEventBasedOnRefundType::creditmemoRef is not valid one :: "+refundKey);
            }
        }
    }
}
